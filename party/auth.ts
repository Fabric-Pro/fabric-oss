// The expected secret is constant for the life of a server process, but
// publish — the hottest path here, one HTTP call per broadcast event from
// Temporal — would otherwise re-hash it on every request. Memoized on the
// secret string so a changed env (tests, dev reloads) recomputes instead of
// comparing against a stale digest.
 ...
 *
 * In production an unset AGENT_SERVICE_SECRET denies instead of comparing
 * against the interpolated string `Bearer undefined`, which an unconfigured
 * server would otherwise accept from any caller sending exactly that.
 *
 * Deliberate difference from `party-cf/src/auth.ts`: this file leaves dev fully
 * permissive even when a secret IS configured. This legacy server only ever
 * runs on the dev path (local Aspire and PartyKit-hosted staging), so
 * tightening its dev branch would change behavior in a live environment.
 * party-cf ships `PARTYKIT_ENV=production` everywhere it is deployed, so its
 * dev branch is local-only and can afford to require a configured secret. The
 * production branches must stay in lockstep — see party/README.md.
 */
async function isServiceRequestAuthorized(
	...
	const expectedSecret = env.AGENT_SERVICE_SECRET as string | undefined;
	if (!expectedSecret) {
		return false;
	}
 ...
 * Mirrors `requireServiceAuth` in party-cf/src/auth.ts.
 */
 ...
 *
 * Mirrors `verifyWithFabric` in party-cf/src/auth.ts — unlike the service-auth
 * gate above there is no dev/prod difference here, because every caller reaches
 * this only from its production branch.
 */
export async function verifyWithFabric(
	...
	const apiUrl = env.FABRIC_API_URL as string | undefined;
	if (!apiUrl) {
