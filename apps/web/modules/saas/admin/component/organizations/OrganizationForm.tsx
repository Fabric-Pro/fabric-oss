...
export function OrganizationForm({
	...
	// Workspace-aware admin path builder — the post-create redirect must keep
	// the current org slug so creating an org from `/app/{slug}/admin` doesn't
	// drop the user back into the personal workspace.
	...
	const onSubmit = form.handleSubmit(async ({ name }) => {
		try {
			...
			if (!organization) {
				router.replace(
					adminPath(`/organizations/${newOrganization.id}`),
				);
			}
		} catch {
