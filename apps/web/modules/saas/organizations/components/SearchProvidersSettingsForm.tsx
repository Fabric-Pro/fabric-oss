...
export function SearchProvidersSettingsForm({
	...
	const { data: providerConfigs = [], isLoading: isLoadingConfigs } =
		useQuery({
			queryKey: ["organizationSearchProviders", organization?.id],
			queryFn: async () => {
				if (!organization?.id) {
					return [];
				}
				return await orpcClient.searchProviders.getOrganizationProviders(
	...
	const updateMutation = useMutation({
		mutationFn: async (data: {
			...
			return await orpcClient.searchProviders.updateOrganizationProvider({
				organizationId: organization.id,
	...
	if (!organization) {
		return (
			<SettingsItem
				...
				description="Configure search providers for your organization."
			>
							...
							<div className="flex gap-3">
								...
								<div className="space-y-1 text-sm">
									...
									<p className="text-highlight/80">
										...
										modify search provider settings.
									</p>
								</div>
							</div>
						</div>
