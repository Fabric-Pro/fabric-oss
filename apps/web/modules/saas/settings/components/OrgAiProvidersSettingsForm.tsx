...
export function OrgAiProvidersSettingsForm({
	readOnly = false,
	...
	// Query AI configuration status for organization
	...
	const saveProviderMutation = useMutation({
		...
		onSuccess: () => {
			...
			// Include organizationId so the query key matches what AiChat uses
			queryClient.invalidateQueries({
				queryKey: ["aiAvailableModels", organizationId],
			});
			queryClient.invalidateQueries({
				queryKey: ["aiTaskDefaults", organizationId],
			});
	...
	const setDefaultMutation = useMutation({
		mutationFn: async (provider: string) => {
			...
			return await orpcClient.aiConfig.providers.setDefault({
				provider: provider as never,
				organizationId: organizationId ?? undefined, // Set default for organization
			});
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["aiConfigStatus"] });
	...
	const setEmbeddingMutation = useMutation({
		mutationFn: async (provider: string) => {
			...
			return await orpcClient.aiConfig.providers.setEmbedding({
				provider: provider as never,
				organizationId: organizationId ?? undefined, // Set for organization
			});
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["aiConfigStatus"] });
	...
	const { data: gatewayConfig } = useQuery({
		...
		queryFn: async () => {
			...
			return await orpcClient.aiConfig.providers.getConfig({
				...
				organizationId: organizationId ?? undefined, // Get org-level config
			});
		},
		enabled: !!selectedGatewayForProviders && !!isOrgContext,
	});
	...
	const updateEnabledMutation = useMutation({
		mutationFn: async (data: {
			...
			return await orpcClient.aiConfig.providers.updateEnabled({
				...
				organizationId: organizationId ?? undefined, // Update org-level config
			});
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["aiConfigStatus"] });
	...
	const handleSetEmbedding = async (providerId: string) => {
		...
		// If there's already an embedding provider, show warning
		if (
	...
	return (
		<>
			<SettingsItem
				...
				description="Configure AI providers for your organization. These settings apply to all organization members unless they have personal overrides."
			>
							...
							<div className="flex items-start gap-3">
								...
								<div className="space-y-1">
									...
									<p className="text-highlight/80 text-sm">
										{readOnly
											? "No AI providers have been configured for this organization yet."
											: "Configure at least one AI provider below to enable AI-powered features for your organization."}
									</p>
								</div>
							</div>
						</div>
								...
								<div className="flex gap-3">
									...
									<div className="space-y-1 text-sm">
										...
										<p className="text-secondary/80">
											...
											This keeps your document index
											stable even when you switch your
											default AI provider for chat. Click{" "}
											<strong>"Use for Documents"</strong>{" "}
											on any configured provider that
											supports embeddings.
										</p>
									</div>
								</div>
			...
			<Dialog
				open={!!selectedProvider && !readOnly}
				onOpenChange={(open) => {
					if (!open) {
						setSelectedProvider(null);
						resetCredentialFields();
				...
				<DialogContent className="max-w-md">
							...
							<div className="space-y-2">
								...
								>
									...
									>
										<RadioGroupItem
											value="apiKey"
											id="org-auth-mode-api-key"
											className="mt-0.5"
										/>
										<Label
											htmlFor="org-auth-mode-api-key"
									...
									>
										<RadioGroupItem
											...
											id="org-auth-mode-service-principal"
											className="mt-0.5"
										/>
										<Label
											htmlFor="org-auth-mode-service-principal"
											className="flex-1 cursor-pointer"
										>
											<div className="font-medium">
												Service principal
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={
								!credentialsComplete ||
								// Not merely "a test passed" — it must have passed
								// for the values currently in the form.
								!testPassedForCurrentInput ||
			...
			<Dialog
				open={enabledProvidersDialogOpen && !readOnly}
				onOpenChange={(open) => {
					if (!open) {
						setEnabledProvidersDialogOpen(false);
						setSelectedGatewayForProviders(null);
			...
			<Dialog
				open={embeddingChangeConfirmOpen && !readOnly}
				onOpenChange={(open) => {
					if (!open) {
						setEmbeddingChangeConfirmOpen(false);
						setPendingEmbeddingProvider(null);
				...
				<DialogContent className="max-w-md">
					<DialogHeader>
						...
						<DialogDescription className="space-y-3">
							<p>
								You're about to change the organization's
								embedding provider from{" "}
								<strong>
									{configStatus?.embeddingProvider?.replace(
										/_/g,
										" ",
							...
							<div className="rounded-md border border-highlight/20 bg-highlight/5 p-3">
								<p className="text-highlight text-sm">
									...
									vectors. Documents indexed with the previous
									provider may not be searchable correctly.
								</p>
								<p className="text-highlight/80 text-sm mt-2">
									Consider re-indexing your organization's
									documents after changing the embedding
									provider.
								</p>
							</div>
						</DialogDescription>
