 ...
 *
 * Mounted by BOTH the personal (`/app/agents/register`) and the organization
 * (`/app/{slug}/agents/register`) route. It reads its tenant and its base path
 * from `useOrganizationContext()`, so registering from inside an organization
 * lands back inside it (Fizzy #1875 R13).
 */
...
export function RegisterExternalAgent() {
	...
	const registerMutation = useMutation({
		...
		onSuccess: () => {
			...
			// The two sibling "back to agents" buttons below already derive
			// their target from `basePath`; this one used to hard-code the
			// personal tree and walked an organization member out of it.
