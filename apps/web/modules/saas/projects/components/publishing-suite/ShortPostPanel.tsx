...
export function ShortPostPanel({
	...
	const refinement = useDraftRefinement({
		...
		confirmAccept: () =>
			!isBodyDirty ||
			window.confirm(
				"Saving the refined short post discards your unsaved edits. Continue?",
			),
	...
	return (
		<div className="space-y-5">
			...
			{working?.hasBody ? (
				<section className="space-y-2">
					...
					<div className="space-y-3 rounded-xl border border-border bg-muted/40 p-4">
										...
										>
											<div className="space-y-1">
												...
												<p className="text-muted-foreground text-xs leading-relaxed">
													...
													proposed revision of that
													text, shown as a diff you
													accept or discard — it does
													not make a new version, and
									...
									{refinement.isRunning ? (
										<output className="flex items-center gap-2 text-muted-foreground text-sm">
											<Loader2Icon
												...
												aria-hidden="true"
											/>
											Revising your saved short post…
										</output>
									) : null}
								</>
							) : null}
						...
						{canEdit ? (
							<p className="text-muted-foreground text-xs leading-relaxed">
								Refine with AI is what edits this short post —
								the AI Assistant works on the planning analysis,
								not on drafts.
							</p>
						) : null}
			...
			{/* Directly under the draft it revises, because that is what it
			    is about: a proposal to change the saved short post, not a
