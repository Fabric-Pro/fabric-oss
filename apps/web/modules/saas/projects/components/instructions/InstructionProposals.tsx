										...
										{filePage.data ? (
											<>
												<pre className="max-h-64 overflow-auto whitespace-pre-wrap rounded-md bg-muted p-2 font-mono text-xs">
													{(filePage.data as ProposalFilePage).body}
												</pre>
												<div className="mt-2 flex gap-2">
													<Button
														variant="outline"
														disabled={filePageInput.offset === 0}
														onClick={() =>
															setFilePageInput((current) =>
																current
																	? {
																			...current,
																			offset: Math.max(0, current.offset - 50_000),
																		}
																	: null,
															)
														}
													...
													<Button
														variant="outline"
														disabled={(filePage.data as ProposalFilePage).nextOffset === null}
														onClick={() => {
															const nextOffset = (filePage.data as ProposalFilePage).nextOffset;
															if (nextOffset !== null) {
																setFilePageInput((current) =>
																	current ? { ...current, offset: nextOffset } : null,
																);
															}
														}}
													>
