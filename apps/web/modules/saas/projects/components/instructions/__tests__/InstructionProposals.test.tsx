		...
		projects: {
			instructions: {
				...
				proposals: {
					...
					file: {
						queryOptions: queryOptions("file", () => state.filePage),
					},
					approve: {
...
describe("InstructionProposals", () => {
	...
	it("clears an open file page when selecting a different proposal", async () => {
		...
		await user.click(screen.getByRole("button", { name: "View full text" }));
		expect(await screen.findByText("old proposal page")).toBeInTheDocument();
		await user.click(screen.getByRole("button", { name: "Proposal version 9" }));
		await waitFor(() =>
