...
interface GitLabProjectPickerProps {
	...
	onConfirm: (repos: GitLabProject[]) => void;
	initialRepos?: GitLabProject[];
	/** When provided, uses project-level shared credentials if the user has no personal GitLab connection */
	projectId?: string;
}
...
export function GitLabProjectPicker({
	...
	const mcpSettingsPathRaw = useContextPath("settings/mcp");
	const buildReturnUrl = useSettingsReturnUrl();
	const mcpSettingsPath = buildReturnUrl(mcpSettingsPathRaw);
	...
	const [groupSearchError, setGroupSearchError] = useState<string | null>(
		null,
	);
	...
	const fetchRepos = async () => {
		...
		setError(null);
		try {
			const result = await orpcClient.projects.gitlab.listRepos({
	...
	const searchGroupProjects = async (orgName: string) => {
		if (!orgName.trim()) {
			return;
		}
		...
		if (
			...
			setGroupSearchError(`"${orgName.trim()}" is already loaded.`);
			return;
		}
		...
		try {
			...
			if (result.groups && result.groups.length > 0) {
				...
				setGroupSearchError(
					`No projects found for "${orgName.trim()}".`,
				);
			}
		} catch (err) {
			setGroupSearchError(
				err instanceof Error ? err.message : "Failed to search group",
			);
		} finally {
	...
	const handleConfirm = () => {
		...
		if (selected.length > 0) {
			...
			const selectedWithTag = selected.map((p) => ({
				...p,
				...(tag ? { roleTag: tag } : {}),
			}));
			onConfirm(selectedWithTag);
			onOpenChange(false);
		}
	};
			...
			<DialogContent className="flex max-h-[80vh] max-w-2xl flex-col overflow-hidden">
				<DialogHeader className="shrink-0">
					<DialogTitle className="flex items-center gap-2">
						<GitLabIconSvg className="size-5" />
						Select GitLab Projects
					</DialogTitle>
					<DialogDescription>
						Search for your GitLab group to browse its repositories.
					</DialogDescription>
					<p className="mt-1 text-xs text-muted-foreground">
						Premium/Ultimate users can also connect GitLab via the
						official MCP in Settings → MCP Servers.
					</p>
				</DialogHeader>
					...
					<div className="flex flex-col items-center gap-4 py-8 text-center">
						...
						<div>
							...
							<p className="mt-1 text-sm text-foreground/50">
								Connect GitLab in MCP Servers to browse and
								select projects for this project.
							</p>
						</div>
						<Button asChild variant="outline">
							<a href={mcpSettingsPath} className="gap-2">
							...
							<div className="shrink-0 space-y-2">
								<div className="flex gap-2">
									<div className="relative flex-1">
										...
										<Input
											placeholder="Enter group name (e.g. Fabric-Pro)..."
								...
								{groups.length === 0 ? (
									<div className="flex flex-col items-center justify-center gap-2 p-8 text-muted-foreground">
										...
										<p className="font-medium">
											Search for a group above
										</p>
										<p className="text-center text-xs">
											Type your GitLab group name and
											press Enter or click "Add Group"
										</p>
									</div>
				...
				<DialogFooter className="shrink-0 border-t pt-4 flex items-center justify-between gap-3">
					...
					<div className="flex items-center gap-2">
						...
						<Button
							...
							className="gap-2"
						>
							<GitLabIconSvg className="size-4" />
							{selectedRepos.size > 0
								? `Add ${selectedRepos.size} Project${selectedRepos.size !== 1 ? "s" : ""}`
								: "Select Projects"}
						</Button>
					</div>
				</DialogFooter>
