...
these three included), shows the same configuration block, and gives you a starter sentence
to paste into your tool:
...
| `fabric_list_project_instructions` | List a project's published coding-instruction files — path, kind, name, description, size |
| `fabric_get_project_instruction` | Read one published coding-instruction file by path, paged for large files |
| `fabric_get_project_instruction_bundle` | Fetch every published coding-instruction file as one zip |
