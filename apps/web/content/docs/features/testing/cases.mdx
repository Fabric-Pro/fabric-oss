...
it records depends on which runner produced it:

- **Agentic runs** record one row per authored step, with what the runner
  observed and — where your evidence policy asks for one — the screenshot it
  captured.
- **Scripted runs** record one row per action in the saved plan: PASSED,
  FAILED (with the expected and received values, when the failing step was an
  assertion) or SKIPPED for the actions after it. Scripted runs capture no
  screenshots — a saved plan is deterministic, so the observation text is the
  full record.

Where a screenshot exists, it is shown **under the step it belongs to**, not
behind a link, so finding the frame where a run went wrong is a matter of
scrolling rather than opening every step in a new tab. Two controls sit
beneath it:
