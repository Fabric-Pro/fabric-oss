...
<Steps>
  ...
  <Step>
    ...
    Adversarial reviewers that add extra cases during planning, each with its own lens — **Security Reviewer**, **UX Skeptic**, **Performance Critic**, **Accessibility Auditor**, and **Edge-Case Hunter**. Only **UX Skeptic** is on in a new project; enable the others when you want their lens. Cases they contribute arrive as **Proposed**, so nothing enters your suite without review.
  ...
  <Step>
    ...
    Under **Confidence & coverage**: the level the automation figure on the Testing tab is measured against. Defaults to **80%**.

    This target is reporting only — it shapes what the coverage rings tell you, and blocks nothing. Turning off **Index coverage** removes the comparison entirely.
  ...
  <Step>
    ...
    Under **Sign-off**: the percentage of a feature's acceptance criteria that must have at least one test case linked before the feature can be marked Done. **Off by default** — switching it on pre-fills **30%**.
    ...
    <Callout type="info">
    Switching the gate **off** — or setting the target to **0** — disables the check entirely. A feature with no acceptance criteria always passes it — there is nothing to cover, and blocking it behind a target it can never reach would help nobody.
    </Callout>
  ...
  <Step>
    ...
    The viewport and browser a Fabric-driven run uses. Defaults are **1920×1080** and **Chrome** — one combination, since that is exactly what a run reads. Firefox and WebKit are available, and any other window size fits the free-text field.
  </Step>
