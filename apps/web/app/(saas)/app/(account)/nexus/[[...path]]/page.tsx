...
/**
 * Everything that used to live under `/app/nexus/**` — the Nexus chat surface.
 *
 ...
 *
 * A catch-all under the static `nexus` segment, not a plain page: it has to
 * answer for `/app/nexus` AND everything below it, and it can only do that
 * from here — see `redirectIntoOrganization` for why the top level cannot.
 */
export default async function PersonalNexusRedirect({
	params,
	searchParams,
	...
	return redirectIntoOrganization("nexus", path, query);
}
