...
/**
 * Everything that used to live under `/app/workflows/**` — workflows.
 *
 ...
 *
 * A catch-all under the static `workflows` segment, not a plain page: it has to
 * answer for `/app/workflows` AND everything below it, and it can only do that
 * from here — see `redirectIntoOrganization` for why the top level cannot.
 */
export default async function PersonalWorkflowsRedirect({
	params,
	searchParams,
	...
	return redirectIntoOrganization("workflows", path, query);
}
