...
/**
 * Everything that used to live under `/app/agent-templates/**` — the agent template gallery.
 *
 ...
 *
 * A catch-all under the static `agent-templates` segment, not a plain page: it has to
 * answer for `/app/agent-templates` AND everything below it, and it can only do that
 * from here — see `redirectIntoOrganization` for why the top level cannot.
 */
export default async function PersonalAgentTemplatesRedirect({
	params,
	searchParams,
	...
	return redirectIntoOrganization("agent-templates", path, query);
}
