...
/**
 * Everything that used to live under `/app/skills/**` — the skill library.
 *
 ...
 *
 * A catch-all under the static `skills` segment, not a plain page: it has to
 * answer for `/app/skills` AND everything below it, and it can only do that
 * from here — see `redirectIntoOrganization` for why the top level cannot.
 */
export default async function PersonalSkillsRedirect({
	params,
	searchParams,
	...
	return redirectIntoOrganization("skills", path, query);
}
