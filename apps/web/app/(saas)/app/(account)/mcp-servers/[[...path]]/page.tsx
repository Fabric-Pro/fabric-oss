...
/**
 * Everything that used to live under `/app/mcp-servers/**` — connected MCP servers.
 *
 ...
 *
 * A catch-all under the static `mcp-servers` segment, not a plain page: it has to
 * answer for `/app/mcp-servers` AND everything below it, and it can only do that
 * from here — see `redirectIntoOrganization` for why the top level cannot.
 */
export default async function PersonalMcpServersRedirect({
	params,
	searchParams,
	...
	return redirectIntoOrganization("mcp-servers", path, query);
}
