...
/**
 * Everything that used to live under `/app/prompts/**` — the prompt library.
 *
 ...
 *
 * A catch-all under the static `prompts` segment, not a plain page: it has to
 * answer for `/app/prompts` AND everything below it, and it can only do that
 * from here — see `redirectIntoOrganization` for why the top level cannot.
 */
export default async function PersonalPromptsRedirect({
	params,
	searchParams,
	...
	return redirectIntoOrganization("prompts", path, query);
}
