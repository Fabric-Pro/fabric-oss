...
/**
 * Everything that used to live under `/app/admin/**` — the system-admin console.
 *
 ...
 *
 * A catch-all under the static `admin` segment, not a plain page: it has to
 * answer for `/app/admin` AND everything below it, and it can only do that
 * from here — see `redirectIntoOrganization` for why the top level cannot.
 */
export default async function PersonalAdminRedirect({
	params,
	searchParams,
	...
	return redirectIntoOrganization("admin", path, query);
}
