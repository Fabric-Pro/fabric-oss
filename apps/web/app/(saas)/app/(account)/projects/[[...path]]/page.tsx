...
/**
 * Everything that used to live under `/app/projects/**` — projects, and everything beneath one.
 *
 ...
 *
 * A catch-all under the static `projects` segment, not a plain page: it has to
 * answer for `/app/projects` AND everything below it, and it can only do that
 * from here — see `redirectIntoOrganization` for why the top level cannot.
 */
export default async function PersonalProjectsRedirect({
	params,
	searchParams,
	...
	return redirectIntoOrganization("projects", path, query);
}
