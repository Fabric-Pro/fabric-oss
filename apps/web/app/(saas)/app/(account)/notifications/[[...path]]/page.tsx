...
/**
 * Everything that used to live under `/app/notifications/**` — the notification centre.
 *
 ...
 *
 * A catch-all under the static `notifications` segment, not a plain page: it has to
 * answer for `/app/notifications` AND everything below it, and it can only do that
 * from here — see `redirectIntoOrganization` for why the top level cannot.
 */
export default async function PersonalNotificationsRedirect({
	params,
	searchParams,
	...
	return redirectIntoOrganization("notifications", path, query);
}
