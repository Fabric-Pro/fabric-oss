...
describe("WebinarScriptPanel — refining the saved draft", () => {
	...
	it("starts a REFINEMENT, never a generation", async () => {
		...
		expect(mutate.refine).toHaveBeenCalledWith({
			...
			instruction: "Cut the demo to five minutes.",
		});
 ...
 */
describe("WebinarScriptPanel — the assistant boundary", () => {
	it("says which AI edits this draft and which one does not", () => {
		renderPanel({ draft: readyDraft(), working: working() as never });

		expect(
			screen.getByText(
				/refine with ai is what edits this webinar script .* the ai assistant works on the planning analysis, not on drafts/i,
			),
		).toBeInTheDocument();
	});
 ...
 */
describe("WebinarScriptPanel — reviewing a refinement proposal", () => {
	const PROPOSED = "Open with the cold-build problem. Then demo.";
	const SAVED = new Date("2026-09-01T12:00:00Z");

	const saved = {
		postType: "WEBINAR_SCRIPT" as const,
		hasBody: true,
		body: "Open with the cold-build problem, then the demo.",
		sourceDraftId: "d1",
		sourceOptionLabel: null,
		sourceContent: null,
		updatedAt: SAVED,
 ...
 */
describe("WebinarScriptPanel — the refine pending state", () => {
	const saved = {
		postType: "WEBINAR_SCRIPT" as const,
		hasBody: true,
		body: "Open with the cold-build problem, then the demo.",
	...
	it("reports a run in flight beside the control that started it", () => {
		...
		expect(screen.getByRole("status")).toHaveTextContent(
			/revising your saved webinar script/i,
		);
	});
