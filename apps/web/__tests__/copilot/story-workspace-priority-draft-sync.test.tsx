/**
 * Local integration test for the V1 (classic) editor's metadata Priority
 * sparkle + DRAFT SYNC. This surface cannot be driven on staging (the org is
 * entirely on the V2 editor) and had no behavioral test. StoryWorkspace holds
 * `priority` in local draft state saved on submit, so an AI move applied via
 * the adjacent sparkle MUST sync that draft or a later Save overwrites it with
 * the stale value. We stub AiReprioritizeControl at its `onApplied` seam and
 * assert the Select reflects the AI's band. Mount scaffold copied from
 * story-workspace-history-mount.test.tsx.
 */
...
vi.mock(
	"../../modules/saas/projects/components/stories/priority/useAiReassessEligibility",
	() => ({ useAiReassessEligibility: () => true }),
);

vi.mock(
	"../../modules/saas/projects/components/stories/priority/AiReprioritizeControl",
	() => ({
		AiReprioritizeControl: ({
			...
			<button
				type="button"
				data-testid="ai-reassess-stub"
				onClick={() =>
					onApplied?.({ changed: true, toPriority: "P0_CRITICAL" })
				}
			>
				ai-reassess
			</button>
...
function priorityText(): string {
	return document.getElementById("priority")?.textContent ?? "";
}

describe("StoryWorkspace — V1 metadata Priority sparkle + draft sync", () => {
	beforeEach(() => {
		mockUseHistoryEnabled.mockReturnValue(false);
	});

	it("renders the V1 metadata Priority field with the AI sparkle beside it", () => {
		renderWorkspace();
		expect(document.getElementById("priority")).not.toBeNull();
		expect(screen.getByTestId("ai-reassess-stub")).toBeInTheDocument();
	});

	it("syncs the form priority draft to an applied AI move", () => {
		renderWorkspace();
		// Before: the story's own band, and definitively NOT the AI's target.
		expect(priorityText()).toMatch(/Medium/i);
		expect(priorityText()).not.toMatch(/Critical/i);

		act(() => {
			fireEvent.click(screen.getByTestId("ai-reassess-stub"));
		});

		// After: the draft reflects the AI's band (P0), and no longer the old one.
		expect(priorityText()).toMatch(/Critical/i);
		expect(priorityText()).not.toMatch(/Medium/i);
	});
});
