...
In this spec's folder, create this file: `fabric/specs/[this-spec]/orchestration.yml`.
...
Loop through each task group in `fabric/specs/[this-spec]/tasks.md` and delegate its implementation to the assigned specialist specified in `orchestration.yml`.

For each delegation, provide the specialist with:
- The task group (including the parent task and all sub-tasks)
- The spec file: `fabric/specs/[this-spec]/spec.md`
- Instruct specialist to:
  - Perform their implementation
  - Check off the task and sub-task(s) in `fabric/specs/[this-spec]/tasks.md`
...
1. **If `standards` contains "all"**:
   - Include all files from `fabric/standards/global/*.md`
   - Include all files from `fabric/standards/frontend/*.md`
   - Include all files from `fabric/standards/backend/*.md`
   - Include all files from `fabric/standards/infrastructure/*.md`

2. **If `standards` contains a wildcard pattern** (e.g., "global/*", "frontend/*"):
   - Include all `.md` files from `fabric/standards/[pattern]/`
   - Example: "backend/*" → all files in `fabric/standards/backend/`

3. **If `standards` contains a specific file path** (e.g., "frontend/components.md"):
   - Include that specific file: `fabric/standards/[path]`
   - Example: "frontend/components.md" → `fabric/standards/frontend/components.md`
...
5. **Format the file list** for the specialist as markdown file references:
   ```
   - @fabric/standards/global/coding-principles.md
...
Follow these steps to generate this spec's ordered series of prompts texts, each in its own .md file located in `fabric/specs/[this-spec]/implementation/prompts/`.

LOOP through EACH task group in `fabric/specs/[this-spec]/tasks.md` and for each, use the following workflow to generate a markdown file with prompt text for each task group:

#### Step 1. Create the prompt markdown file

Create the prompt markdown file using this naming convention:
`fabric/specs/[this-spec]/implementation/prompts/[task-group-number]-[task-group-title].md`.
...
2. **Resolve each standards entry to actual file paths** using the same logic from "Building the Standards File List for a Task Group" above:
   ...
   - "global/*" → list all files from `fabric/standards/global/`
   - "frontend/components.md" → `fabric/standards/frontend/components.md`
...
- @fabric/standards/global/coding-principles.md
- @fabric/standards/global/error-handling.md
- @fabric/standards/global/testing.md
- @fabric/standards/global/security.md
- @fabric/standards/frontend/components.md
```
...
Read @fabric/specs/[this-spec]/spec.md to understand the context for this spec and where the current task fits into it.

Also read these further context and reference:
- @fabric/specs/[this-spec/]/planning/requirements.md
- @fabric/specs/[this-spec/]/planning/visuals
...
Progress will be tracked in `fabric/specs/[this-spec]/tasks.md`
```
