...
| `eks` | EKS cluster + managed node group + OIDC provider (dev: m6i.large x2, 40Gi root volumes). Refuses node types that do not encrypt traffic between instances (`require_encryption_in_transit`) |
...
`modules/eks/tests/` holds `terraform test` cases for the node-type encryption guard. They run offline against a mocked AWS provider — no credentials, nothing provisioned:

```bash
terraform -chdir=deploy/terraform/modules/eks init -backend=false
terraform -chdir=deploy/terraform/modules/eks test
```
