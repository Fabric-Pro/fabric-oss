...
output "s3_buckets" {
  description = "Fully-qualified bucket names keyed to match the Helm chart's camelCase s3.buckets.*. Every name is \"<prefix>-<suffix>\"; set that shared prefix as GitLab CI variable S3_BUCKET_PREFIX and ci/gitlab/60-deploy-aws.yml derives all seven."
  value = {
...
output "route53_name_servers" {
  description = "NS records to delegate `domain_name` at your registrar (production terminates TLS at the ALB via the ACM cert on this zone)."
