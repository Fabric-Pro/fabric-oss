 ...
 */
const TRIM_CHARS = Prisma.sql`E' \t\n\r\f\x0B\u00A0\u1680\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200A\u2028\u2029\u202F\u205F\u3000\uFEFF'`;

/** The occurrence's own subject, blank-normalised to NULL. */
const OCCURRENCE_SUBJECT = Prisma.sql`NULLIF(BTRIM(tr."meetingSubject", ${TRIM_CHARS}), '')`;

/** The recurring series' subject, blank-normalised to NULL. */
const SERIES_SUBJECT = Prisma.sql`NULLIF(BTRIM(lm."subject", ${TRIM_CHARS}), '')`;
	...
	>(Prisma.sql`
		...
		scoped AS (
			...
			SELECT
				...
				COALESCE(
					NULLIF(${OCCURRENCE_SUBJECT}, 'Untitled Meeting'),
					NULLIF(${SERIES_SUBJECT}, 'Untitled Meeting'),
					${OCCURRENCE_SUBJECT},
					${SERIES_SUBJECT}
				) AS "meetingTitle",
