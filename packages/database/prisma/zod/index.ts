...
export const ProjectReadinessVerdictScalarFieldEnumSchema = z.enum(['id', 'projectId', 'itemKey', 'isComplete', 'isVisible', 'visibleChangedAt', 'changedAt', 'userId', 'organizationId', 'createdAt', 'updatedAt'])
...
export const ProjectLinkedMeetingScalarFieldEnumSchema = z.enum(['id', 'projectId', 'joinUrl', 'subject', 'organizer', 'includedInDigest', 'linkedAt', 'deactivatedAt', 'deactivatedById', 'consecutiveFailures', 'lastErrorMessage', 'lastErrorAt', 'userId', 'organizationId'])
...
export type DeletedMeetingArchiveScalarFieldEnum = z.infer<typeof DeletedMeetingArchiveScalarFieldEnumSchema>;
...
// File: PublishingTopicQuestionAssigneeScalarFieldEnum.schema.ts

export const PublishingTopicQuestionAssigneeScalarFieldEnumSchema = z.enum(['id', 'decisionEntryId', 'assigneeUserId', 'assignedByUserId', 'projectId', 'userId', 'organizationId', 'createdAt'])

export type PublishingTopicQuestionAssigneeScalarFieldEnum = z.infer<typeof PublishingTopicQuestionAssigneeScalarFieldEnumSchema>;
...
export const ProjectSchema = z.object({
  ...
  pmStatusSyncEnabled: z.boolean(),
  pmStatusSyncSessionAt: z.date().nullish(),
  pmStatusSyncLastRun: z.unknown().refine((val) => { const getDepth = (obj: unknown, depth: number = 0): number => { if (depth > 10) return depth; if (obj === null || typeof obj !== 'object') return depth; const values = Object.values(obj as Record<string, unknown>); if (values.length === 0) return depth; return Math.max(...values.map(v => getDepth(v, depth + 1))); }; return getDepth(val) <= 10; }, "JSON nesting depth exceeds maximum of 10").nullish(),
  ...
  meetingTranscriptSyncUserId: z.string().nullish(),
...
export const ProjectReadinessVerdictSchema = z.object({
  ...
  isVisible: z.boolean(),
  visibleChangedAt: z.date().nullish(),
  changedAt: z.date(),
  userId: z.string().nullish(),
  organizationId: z.string().nullish(),
  createdAt: z.date(),
...
export const DeletedMeetingArchiveSchema = z.object({
  id: z.string(),
  projectId: z.string(),
  joinUrl: z.string(),
  subject: z.string().nullish(),
...
export type DeletedMeetingArchiveType = z.infer<typeof DeletedMeetingArchiveSchema>;
...
export const UserStorySchema = z.object({
  ...
  pmStatusSyncBaseId: z.string().nullish(),
  pmStatusSyncBaseAt: z.date().nullish(),
  pmStatusSyncBaseLink: z.string().nullish(),
  pmStatusSyncBaseFabricId: z.string().nullish(),
...
// File: PublishingTopicQuestionAssignee.schema.ts

export const PublishingTopicQuestionAssigneeSchema = z.object({
  id: z.string(),
  decisionEntryId: z.string(),
  assigneeUserId: z.string(),
  assignedByUserId: z.string(),
  projectId: z.string(),
  userId: z.string().nullish(),
...
export type PublishingTopicQuestionAssigneeType = z.infer<typeof PublishingTopicQuestionAssigneeSchema>;
