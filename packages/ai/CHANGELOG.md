# @repo/ai

## 0.0.3

### Patch Changes

- Updated dependencies [9e8e50e]
- Updated dependencies [bf41584]
  - @repo/database@0.0.3
  - @repo/utils@0.0.1
  - @repo/payments@0.0.3
  - @repo/logs@0.0.1
  - @repo/storage@0.0.1

## 0.0.2

### Patch Changes

- Updated dependencies [2a6e543]
  - @repo/database@0.0.2
  - @repo/payments@0.0.2

## 0.0.1

### Patch Changes

- 1a8fc05: AI Update: classify retried / `RetryError`-wrapped provider errors (#1681).
- 6f8e223: Azure AI Foundry: disable strict JSON-schema structured outputs at the model-factory level so every `generateObject` caller works (not just AI Update).
- Updated dependencies [044a9f7]
- Updated dependencies [3d36eb3]
- Updated dependencies [5ad88a5]
  - @repo/database@0.0.1
  - @repo/payments@0.0.1
