...
describe("stripUnsupportedRequestFields", () => {
	...
	it("drops temperature (Databricks Claude rejects the parameter)", () => {
		const body = JSON.stringify({
			model: "m",
			temperature: 0,
			messages: [],
	...
	it("relaxes strict json_schema (Bug #1681)", () => {
		const body = JSON.stringify({
			model: "m",
			response_format: {
				type: "json_schema",
...
describe("applyDatabricksChatBodyCompat", () => {
	it("strips fields, drops temperature, and relaxes strict json_schema in one pass", () => {
		...
		expect(applyDatabricksChatBodyCompat(body)).toBe(true);
		expect(body.stream_options).toBeUndefined();
		expect(body.parallel_tool_calls).toBeUndefined();
		expect(body.temperature).toBeUndefined();
		// max_tokens is preserved — Anthropic/Databricks require it.
	...
	it("leaves max_tokens untouched", () => {
		const body: Record<string, unknown> = { model: "m", max_tokens: 512 };
		expect(applyDatabricksChatBodyCompat(body)).toBe(false);
		expect(body.max_tokens).toBe(512);
	});

	it("does not touch a non-strict json_schema", () => {
		const body: Record<string, unknown> = {
			model: "m",
			response_format: {
				type: "json_schema",
				json_schema: { name: "s", strict: false, schema: {} },
			},
		};
		expect(applyDatabricksChatBodyCompat(body)).toBe(false);
	});

	it("ignores json_object response_format (no strict field)", () => {
		const body: Record<string, unknown> = {
			model: "m",
	...
	it("returns false when there is nothing to change", () => {
...
describe("createDatabricksFetch", () => {
	...
	it("normalizes array message.content on a valid non-streaming JSON response", async () => {
		...
		const res = await dbFetch(
			"https://x.cloud.databricks.com/serving-endpoints",
			{ method: "POST", body: "{}" },
		);
...
// =============================================================================
// Bug #1942: <think>-tag reasoning stripping for the CopilotKit / agent paths
// (DeepSeek-R1 over Databricks leaks raw <think> tags on the OpenAI-compatible
// surface). Opt-in via `stripReasoning`; the @ai-sdk getModel path does NOT use
// it (it extracts reasoning via extractReasoningMiddleware instead).
// =============================================================================
