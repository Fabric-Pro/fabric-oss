...
describe("MCP Input Schema Normalization", () => {
	it("should unwrap jsonSchema-wrapped MCP input schemas", () => {
		const wrapped = {
			jsonSchema: {
				type: "object",
				properties: {
		...
		expect(unwrapMcpInputSchema(wrapped)).toEqual(wrapped.jsonSchema);
	});

	it("should preserve already-unwrapped schemas", () => {
		const raw = {
			type: "object",
			properties: {
		...
		expect(unwrapMcpInputSchema(raw)).toEqual(raw);
	});

	it("should provide an empty object schema when missing", () => {
		expect(unwrapMcpInputSchema()).toEqual({
			type: "object",
			properties: {},
		});
	});
