...
export const setSubscriptionSeats: SetSubscriptionSeats = async ({
	...
	const subscription = await withProviderBreaker(
		"stripe",
		"subscription_retrieve",
		() => stripeClient.subscriptions.retrieve(id),
	);
	...
	await withProviderBreaker("stripe", "subscription_update", () =>
		stripeClient.subscriptions.update(id, {
			items: [
				{
...
export const cancelSubscription: CancelSubscription = async (id) => {
	...
	await withProviderBreaker("stripe", "subscription_cancel", () =>
		stripeClient.subscriptions.cancel(id),
	);
};
...
export const webhookHandler: WebhookHandler = async (req) => {
	...
	try {
		// `constructEventAsync` performs LOCAL crypto signature
		// verification — no network round-trip — so it intentionally
		// stays outside the breaker. The follow-up `retrieve`/`upsert`
		// calls that DO hit Stripe are wrapped individually below.
	...
	try {
		switch (event.type) {
			case "checkout.session.completed": {
				...
				const checkoutSession = await withProviderBreaker(
					"stripe",
					"checkout_session_retrieve",
					() =>
						stripeClient.checkout.sessions.retrieve(id, {
							expand: ["line_items"],
						}),
				);
