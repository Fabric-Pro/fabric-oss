/**
 * Databricks Model Serving / Unity AI Gateway compatibility shims — agent-bundle
 * copy.
 *
 * This is a faithful mirror of `@repo/ai/lib/databricks-compat.ts`. The LangGraph
 * agent bundle deliberately must NOT import `@repo/ai` — that would pull the whole
 * Vercel AI SDK into every agent's tsup bundle — so the shim is duplicated here.
 * The two copies must be kept in sync; when a new Databricks quirk is fixed,
 * update BOTH files.
 *
 * Databricks' OpenAI-compatible endpoint diverges from the strict OpenAI schema in
 * two ways that break streaming chat through `@langchain/openai`'s ChatOpenAI:
 *
 *  1. REQUEST — `@langchain/openai` sends `stream_options:{include_usage:true}` on
 *     every streaming request (default `streamUsage`). Several Databricks serving
 *     backends reject unknown fields with `400 BAD_REQUEST: json: unknown field
 *     "stream_options"`. We strip it.
 *
 ...
 *     `{type:"reasoning", summary:[...]}`), not a string. `@langchain/openai`'s
 *     completions stream loop (`_streamResponseChunks`) drops any chunk whose
 *     `content` is not a string ("Received non-string content from OpenAI. This is
 *     currently not supported." → `continue`), so the agent receives ZERO text
 *     tokens even after the 400 is fixed. We flatten the array to the concatenated
 *     text of its `type:"text"` blocks (reasoning blocks carry no user-facing text
 *     — the thinking is an opaque `signature` — so they are dropped).
 *
 ...
 *     the string `content` (no native `reasoning_content`). The agent streams the
 *     content verbatim, so it would leak the reasoning into the answer (Bug #1942)
 *     unless we strip those spans. `langchain-models.ts` gates `stripReasoning`
 *     on the resolved *canonical* model identity (via `config.isReasoningModel`,
 *     falling back to {@link isReasoningModelName} on the alias) — NOT the raw
 *     serving-endpoint alias — so a non-R1 model's literal `<think>` markup
 *     (e.g. Claude/Llama emitting it in code/docs) is preserved, and an R1
 *     endpoint mapped to an opaque alias is still stripped.
 *
 *  4. REQUEST (Claude only) — see {@link applyDatabricksPromptCacheMarkers}.
 *     Databricks-served Claude supports Anthropic prompt caching, but only when
 *     the request carries `cache_control` markers; `@langchain/openai` has no way
 *     to emit them, so we inject them here. Shared verbatim with `@repo/ai`'s
 *     copy.
 *
 * The fixes are applied via a custom `fetch` wired into the DATABRICKS branch of
 * `createProviderModel` (see `langchain-models.ts`).
 */

/**
 * Top-level request-body fields that Databricks' OpenAI-compatible serving
 * endpoints reject with HTTP 400 under their strict schema, but that
 * `@langchain/openai`'s ChatOpenAI emits anyway:
 *
 *  - `stream_options` — added on every streaming request (default `streamUsage`);
 *    Databricks: `json: unknown field "stream_options"`.
 *  - `parallel_tool_calls` — added whenever tools are bound; Databricks Claude:
 *    `parallel_tool_calls: Extra inputs are not permitted`.
 *
 * (This is a superset of `@repo/ai`'s copy, which only strips `stream_options` —
 * the Vercel `@ai-sdk` path doesn't emit `parallel_tool_calls`. Stripping the
 * field just lets the model use its default parallel-tool behavior.)
 */
...
// Mirrored verbatim from `@repo/ai`'s databricks-compat.ts — keep in sync.
// ---------------------------------------------------------------------------
 ...
 * and the entire prompt is re-billed every turn. `@langchain/openai` has no notion
 * of Anthropic cache markers and drops them during OpenAI-shaped serialization, so
 * the only place the marker can reach the wire on this path is the body-rewrite
 * layer, here. This matters most for the agent loop, which resends a large system
 * prompt plus the whole tool-call history on every step.
 *
...
export function applyDatabricksPromptCacheMarkers(
	...
	try {
		...
		// Never let a surprising body shape cost us the request — the field strip
		// has already run and its result must still go out.
		return false;
	}
}

/**
 * Remove request-body fields Databricks serving endpoints reject, and inject
 * Anthropic prompt-cache breakpoints for Claude endpoints
 * ({@link applyDatabricksPromptCacheMarkers}). Returns the (possibly rewritten)
 * body string; input is returned unchanged when it isn't JSON or nothing needed
 * rewriting.
 */
 ...
 * in `@repo/ai`'s model-factory.ts). Only these models emit `<think>` reasoning
 * tags, so reasoning stripping is gated to them: a non-R1 model (Claude, Llama)
 * may legitimately output literal `<think>` markup (e.g. in code or docs), which
 * must be preserved, not silently removed.
 */
 ...
 * `<think>` reasoning tags should be stripped.
 *
 * IMPORTANT: pass the resolved *canonical* model name, NOT a Databricks serving-
 * endpoint alias — an opaque alias like `prod-chat` reveals nothing about the
 * backend family, so sniffing it would miss an R1 endpoint mapped to that alias
 * (Bug #1942 review). `createProviderModel`'s Databricks branch prefers the
 * explicit `config.isReasoningModel` signal and only falls back to this check on
 * the alias when the canonical isn't propagated.
 */
 ...
 * `reasoning_content`). The LangChain agent path streams this content verbatim,
 * so it would leak the raw reasoning into the answer (Bug #1942) unless we strip
 * it. Enabled via the `stripReasoning` option on `createDatabricksFetch`.
 */
 ...
 * the gateway *did* send. That is why the agents' malformed-history 400s read as
 * opaque for weeks: the wire carried
 *
 ...
 *
 * Mirrors `@repo/ai`'s copy — keep the two in sync.
 *
