...
export const PROJECT_FEATURE_GET_INPUT_SCHEMA = {
	...
	properties: {
		feature: {
			type: "string",
			description:
				"Feature identifier (e.g. 'F-040') or id from fabric_list_project_features.",
		},
	},
	required: ["feature"],
} as const satisfies Record<string, unknown>;
...
export const PROJECT_FEATURE_LIST_DESCRIPTION =
	"List the attached project's roadmap items (features and bugs) LIVE from Fabric — identifier (e.g. F-040), title, status, priority, drafting stage, task progress and a short description. Use this — never project_rag_query — for what is on the roadmap, a feature's current status, or which features are in a status or priority. Filter by status name, priority, kind or search text; page with offset when hasMore is true.";

export const PROJECT_FEATURE_GET_DESCRIPTION =
	"Read one feature (or bug) of the attached project LIVE from Fabric: full description, acceptance criteria, status, priority and every task with its completion. Pass the identifier the user mentions (e.g. 'F-040') or an id from fabric_list_project_features.";
