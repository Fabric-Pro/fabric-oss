...
import { PublishingTopicPostType } from "@repo/database";
...
/**
 * The workflow file that generates each content type.
 *
 * Typed as a TOTAL `Record` over `PublishingTopicPostType` deliberately: a new
 * member of that enum fails to compile until it is mapped here, and this
 * package type-checks its tests (`tsconfig.json` includes `**\/*.ts`), so the
 * failure arrives at the same moment the content type does.
 *
 * It replaces two hand-bumped numeric floors — `files.length >= 6` and
 * `>= 7` — which went blind to DELETION. Eight files sit in this directory, so
 * removing any one of them left seven, every `toContain` named a file that was
 * still there, and both floors passed: the suite stayed green with a shipped
 * content type unregistered, which is the exact failure this guard exists to
 * catch. A derived set cannot drift that way, because it is not a count.
 *
 * `TWEET` maps to the short-post workflow: the enum member kept its original
 * name while the format was renamed Short Post everywhere else.
 */
const WORKFLOW_FILE_BY_POST_TYPE: Record<PublishingTopicPostType, string> = {
	TWEET: "generate-publishing-short-post.ts",
	BLOG_POST: "generate-publishing-blog-post.ts",
	CASE_STUDY: "generate-publishing-case-study.ts",
	STAKEHOLDER_EMAIL: "generate-publishing-stakeholder-email.ts",
	LINKEDIN_POST: "generate-publishing-linkedin-post.ts",
	WEBINAR_SCRIPT: "generate-publishing-webinar-script.ts",
	NEWSLETTER_BLURB: "generate-publishing-newsletter-blurb.ts",
};

/**
 * The planning analysis is generated by a publishing workflow too, and it is
 * not a post type — it is what RAISES the questions the post types are written
 * around. Named separately so the `Record` above stays exactly the enum.
 */
const PLANNING_ANALYSIS_WORKFLOW_FILE =
	"generate-publishing-planning-analysis.ts";

const EXPECTED_PUBLISHING_WORKFLOW_FILES = [
	...Object.values(WORKFLOW_FILE_BY_POST_TYPE),
	PLANNING_ANALYSIS_WORKFLOW_FILE,
];
...
describe("discovery preconditions — the guard cannot pass vacuously (workflow registration)", () => {
	...
	it("finds the publishing workflow files it is supposed to find", () => {
		// Named by two long-standing files deliberately: this precondition
		// isolates the WALKER, so a failure here always means discovery broke,
		// never that the expected set below is out of date. The count lives in
		// the registration test, which owns the derived set.
...
describe("publishing workflow registration", () => {
	it("every publishing workflow is a runtime export of the barrel", () => {
		...
		// save it.
		const files = discoverPublishingWorkflowFiles();
		// EXACT set, in both directions. `>=` passed after a deletion; equality
		// fails whether a mapped workflow left the directory or an unmapped one
		// appeared in it, and the message names the file either way.
		expect([...files].sort()).toEqual(
			[...EXPECTED_PUBLISHING_WORKFLOW_FILES].sort(),
		);
