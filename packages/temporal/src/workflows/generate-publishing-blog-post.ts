/**
 * Fire-and-forget workflow that drafts the Blog Post for ONE publishing topic
 * (Publishing Suite Phase 2B-3, Fizzy #1853).
 *
 * Started (not awaited) by `publishingSuite.generateBlogPost`. The caller uses a
 * deterministic, reject-duplicates workflowId (`publishing-topic-bp:<draftId>`)
...
const { generateBlogPostActivity } = proxyActivities<typeof activities>({
	// Provenance-scoped reads, up to 20 GitHub PR fetches, and one COMPLEX-tier
	// LLM call over a long prompt producing long-form output.
...
const { markBlogPostFailedActivity } = proxyActivities<typeof activities>({
	startToCloseTimeout: "30s",
	retry: { maximumAttempts: 3 },
});

export interface GeneratePublishingBlogPostWorkflowInput {
	draftId: string;
	topicId: string;
	projectId: string;
	...
	/**
	 * The topic's saved working blog post when this run REFINES it rather than
...
export interface GeneratePublishingBlogPostWorkflowOutput {
	status: "READY" | "FAILED" | "SUPERSEDED";
	/** Whether this run created the topic's working draft (DV5/FR21). */
	seededWorkingDraft: boolean;
}

export async function generatePublishingBlogPostWorkflow(
	input: GeneratePublishingBlogPostWorkflowInput,
): Promise<GeneratePublishingBlogPostWorkflowOutput> {
	try {
		const result = await generateBlogPostActivity({
		...
		log.error("[publishing-blog-post] generation failed", {
			errorClass,
			detail,
			draftId: input.draftId,
			topicId: input.topicId,
		...
		try {
			await markBlogPostFailedActivity({
				draftId: input.draftId,
				projectId: input.projectId,
				message,
			});
			...
			log.error("[publishing-blog-post] could not mark draft failed", {
				draftId: input.draftId,
				message:
