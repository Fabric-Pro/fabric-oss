/**
 * Fire-and-forget workflow that drafts the Stakeholder Email for ONE publishing
 * topic (Publishing Suite Phase 2C, Fizzy #1854).
 *
 * Started (not awaited) by `publishingSuite.generateStakeholderEmail`. The
 * caller uses a deterministic, reject-duplicates workflowId
 * (`publishing-topic-se:<draftId>`) so concurrent clicks cannot double-spend the
 ...
 * `generatePublishingStakeholderEmailWorkflow`. `fetch:replay-histories`
...
const { generateStakeholderEmailActivity } = proxyActivities<typeof activities>(
	{
		// Provenance-scoped reads, up to 20 GitHub PR fetches, and one
		// COMPLEX-tier LLM call over the same wide prompt the case study uses.
		// The OUTPUT is much shorter — an email, not a long-form narrative — but
		// the timeout is dominated by the context assembly and the model's
		// time-to-first-token, so trimming it would buy nothing and would turn a
		// slow provider into a spurious failure.
...
const { markStakeholderEmailFailedActivity } = proxyActivities<
	typeof activities
>({
...
export interface GeneratePublishingStakeholderEmailWorkflowInput {
	draftId: string;
	topicId: string;
	projectId: string;
	...
	/**
	 * The topic's saved working stakeholder email when this run REFINES it rather than
...
export interface GeneratePublishingStakeholderEmailWorkflowOutput {
	status: "READY" | "FAILED" | "SUPERSEDED";
	/** Whether this run created the topic's working draft (DV5/FR21). */
	seededWorkingDraft: boolean;
}

export async function generatePublishingStakeholderEmailWorkflow(
	input: GeneratePublishingStakeholderEmailWorkflowInput,
): Promise<GeneratePublishingStakeholderEmailWorkflowOutput> {
	try {
		const result = await generateStakeholderEmailActivity({
		...
		if (result.status === "SUPERSEDED") {
			log.info("[publishing-stakeholder-email] attempt did not commit", {
				draftId: input.draftId,
				topicId: input.topicId,
		...
		log.error("[publishing-stakeholder-email] generation failed", {
			errorClass,
			detail,
			draftId: input.draftId,
			topicId: input.topicId,
		...
		try {
			await markStakeholderEmailFailedActivity({
				draftId: input.draftId,
				projectId: input.projectId,
				message,
			});
			...
			log.error(
				"[publishing-stakeholder-email] could not mark draft failed",
				{
					draftId: input.draftId,
					message:
