/**
 * Fire-and-forget workflow that drafts the Webinar / Demo Script for ONE
 * publishing topic (Publishing Suite Phase 2D-1, Fizzy #1988).
 *
 * The `generateWebinarScript` procedure (`packages/api/modules/projects/
 * procedures/publishing-suite/webinar-script.ts`) starts it fire-and-forget
 * with a deterministic, reject-duplicates workflowId
 * (`publishing-topic-ws:<draftId>`, spec §3) so
 * concurrent clicks cannot double-spend the LLM call — the draft id is
 ...
 * retry. Same contract as `generate-publishing-case-study.ts`.
 *
 ...
 * `generatePublishingWebinarScriptWorkflow`. `fetch:replay-histories` discovers
...
const { generateWebinarScriptActivity } = proxyActivities<typeof activities>({
	// DB reads (the topic, its effective planning analysis, its decisions,
	// its contributors' names) and one COMPLEX-tier LLM call producing
	// long-form script output.
...
const { markWebinarScriptFailedActivity } = proxyActivities<typeof activities>({
	startToCloseTimeout: "30s",
	retry: { maximumAttempts: 3 },
});

export interface GeneratePublishingWebinarScriptWorkflowInput {
	draftId: string;
	topicId: string;
	projectId: string;
...
export interface GeneratePublishingWebinarScriptWorkflowOutput {
	status: "READY" | "FAILED" | "SUPERSEDED";
	/** Whether this run created the topic's working draft (DV5/FR21). */
	seededWorkingDraft: boolean;
}

export async function generatePublishingWebinarScriptWorkflow(
	input: GeneratePublishingWebinarScriptWorkflowInput,
): Promise<GeneratePublishingWebinarScriptWorkflowOutput> {
	try {
		const result = await generateWebinarScriptActivity({
		...
		if (result.status === "SUPERSEDED") {
			log.info("[publishing-webinar-script] attempt did not commit", {
				draftId: input.draftId,
				topicId: input.topicId,
		...
		log.error("[publishing-webinar-script] generation failed", {
			errorClass,
			detail,
			draftId: input.draftId,
			topicId: input.topicId,
		...
		try {
			await markWebinarScriptFailedActivity({
				draftId: input.draftId,
				projectId: input.projectId,
				message,
			});
			...
			log.error(
				"[publishing-webinar-script] could not mark draft failed",
				{
					draftId: input.draftId,
					message:
