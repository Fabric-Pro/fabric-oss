/**
 * Fire-and-forget workflow that drafts the Newsletter Blurb for ONE publishing
 * topic (Publishing Suite Phase 2D slice 2D-2, Fizzy #1988).
 *
 * The `generateNewsletterBlurb` procedure starts it fire-and-forget with a
 * deterministic, reject-duplicates workflowId (`publishing-topic-nb:<draftId>`,
 * spec §3) so concurrent clicks cannot double-spend the LLM call — the draft id
 * is already unique per attempt, and the partial unique index on
 * `status = 'GENERATING'` per content type is what makes only one attempt
 * exist.
 *
 ...
 * retry. Same contract as `generate-publishing-webinar-script.ts`.
 *
 ...
 * `generatePublishingNewsletterBlurbWorkflow`. `fetch:replay-histories`
 * discovers fixtures from workflow types that have actually RUN in dev, and a
 * workflow introduced by this change has never run anywhere, so the gate
 * contributes nothing for it. What determinism rests on until then is the unit
 * test beside this file, which drives the function directly with
 * `@temporalio/workflow` mocked — that catches a wrong branch, not a wrong
 * command sequence.
 *
...
const { generateNewsletterBlurbActivity } = proxyActivities<typeof activities>({
	// DB reads (the topic, its effective planning analysis, its decisions,
	// its contributors' names) and one COMPLEX-tier LLM call.
...
const { markNewsletterBlurbFailedActivity } = proxyActivities<
	typeof activities
>({
...
export interface GeneratePublishingNewsletterBlurbWorkflowInput {
	draftId: string;
	topicId: string;
	projectId: string;
...
export interface GeneratePublishingNewsletterBlurbWorkflowOutput {
	status: "READY" | "FAILED" | "SUPERSEDED";
	/** Whether this run created the topic's working draft (DV5/FR21). */
	seededWorkingDraft: boolean;
}

export async function generatePublishingNewsletterBlurbWorkflow(
	input: GeneratePublishingNewsletterBlurbWorkflowInput,
): Promise<GeneratePublishingNewsletterBlurbWorkflowOutput> {
	try {
		const result = await generateNewsletterBlurbActivity({
		...
		//
		// The STATUS is deliberately still "SUPERSEDED" for every refusal; the
		// specific reason travels separately as the optional `refusalReason`.
		// Renaming the status later would change a branch condition and break
		// replay for any execution already in flight at that point (TMPRL1100),
		// so a future rename must be gated with `patched()`, same as any other
		// change to this workflow's command sequence.
		if (result.status === "SUPERSEDED") {
			log.info("[publishing-newsletter-blurb] attempt did not commit", {
				draftId: input.draftId,
				topicId: input.topicId,
		...
		log.error("[publishing-newsletter-blurb] generation failed", {
			errorClass,
			detail,
			draftId: input.draftId,
			topicId: input.topicId,
		...
		try {
			await markNewsletterBlurbFailedActivity({
				draftId: input.draftId,
				projectId: input.projectId,
				message,
			});
			...
			log.error(
				"[publishing-newsletter-blurb] could not mark draft failed",
				{
					draftId: input.draftId,
					message:
