/**
 * The failure marker for a short post attempt (#1853, Phase 2B-2).
 *
...
export interface MarkShortPostFailedInput {
	draftId: string;
	projectId: string;
	message: string;
}

export async function markShortPostFailedActivity(
	input: MarkShortPostFailedInput,
): Promise<void> {
	...
	if (!persisted) {
		logger.info(
			"[publishing-short-post] failure marker skipped; attempt was already terminal",
