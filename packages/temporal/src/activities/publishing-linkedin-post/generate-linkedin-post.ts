/**
 * LinkedIn Post — the LLM activity (Fizzy #1851).
 *
 * One model call, wrapped in the same guards its Short Post / Tweet sibling
 ...
 *     project permission and NOT org membership. Provider resolution is
 *     organization-first, so a revoked collaborator would otherwise keep
 *     spending the organization's key and credits on its material. Fail-closed
 *     via a non-retryable throw.
 ...
 *     it as a finished answer and "exactly three" would be broken with nothing
 *     downstream ever noticing.
 *
...
export async function generateLinkedInPostActivity(
	...
	heartbeat(`linkedInPost: ${draftId}`);
	...
	await assertGenerationActorAuthorized({
		...
		organizationId,
		actorUserId,
		activity: "generateLinkedInPostActivity",
	});
	...
	const [
		...
		getBoundPromptForAgent({
			agentName: PUBLISHING_LINKEDIN_POST_AGENT_KEY,
		...
		// answer, not a failure — a topic nobody has analysed still drafts from
		// its raw sources.
		getEffectivePlanningAnalysis({ topicId, projectId }),
	]);

	heartbeat(`linkedInPost: context assembled for ${draftId}`);
	...
	const decisions: LinkedInPostDecision[] = [];
	...
	try {
		result = await generateObject({
			model,
			schema: PublishingLinkedInPostSchema,
			prompt,
		...
		if (NoObjectGeneratedError.isInstance(error)) {
			throw ApplicationFailure.nonRetryable(
				`LinkedIn post failed schema validation: ${error.message}`,
	...
	const parsed = PublishingLinkedInPostSchema.safeParse(result.object);
	if (!parsed.success) {
		throw ApplicationFailure.nonRetryable(
			`LinkedIn post failed schema validation: ${parsed.error.message}`,
			"PUBLISHING_LINKEDIN_POST_SCHEMA_VALIDATION_FAILED",
		);
	}
	...
	const content = {
		...
		generation: {
			...
			// the same either way. The body itself is NOT stored: it is already
			// on the working draft row, and a copy per attempt would duplicate
			// the whole post on every refine.
	...
	if (!commit.persisted) {
		...
		logDraftRefusal(
			"[publishing-linkedin-post] draft not committed",
			commit.reason,
			{
				draftId,
				topicId,
