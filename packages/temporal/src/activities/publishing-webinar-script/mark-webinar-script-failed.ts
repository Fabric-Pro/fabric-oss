/**
 * The failure marker for a webinar / demo script attempt (#1988, Phase 2D-1).
 *
...
export interface MarkWebinarScriptFailedInput {
	draftId: string;
	projectId: string;
	message: string;
}

export async function markWebinarScriptFailedActivity(
	input: MarkWebinarScriptFailedInput,
): Promise<void> {
	...
	if (!commit.persisted) {
		...
		logDraftRefusal(
			"[publishing-webinar-script] failure marker skipped",
			commit.reason,
			{
