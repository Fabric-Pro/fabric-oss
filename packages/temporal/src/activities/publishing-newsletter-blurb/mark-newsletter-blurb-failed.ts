/**
 * The failure marker for a Newsletter Blurb attempt (Fizzy #1988, Phase 2D
 * slice 2D-2).
 *
...
export interface MarkNewsletterBlurbFailedInput {
	draftId: string;
	projectId: string;
	message: string;
}

export async function markNewsletterBlurbFailedActivity(
	input: MarkNewsletterBlurbFailedInput,
): Promise<void> {
	...
	if (!commit.persisted) {
		...
		logDraftRefusal(
			"[publishing-newsletter-blurb] failure marker skipped",
			commit.reason,
			{
