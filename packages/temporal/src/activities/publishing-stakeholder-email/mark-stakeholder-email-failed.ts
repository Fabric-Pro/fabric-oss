/**
 * The failure marker for a stakeholder email attempt (#1854, Phase 2C).
 *
...
export interface MarkStakeholderEmailFailedInput {
	draftId: string;
	projectId: string;
	message: string;
}

export async function markStakeholderEmailFailedActivity(
	input: MarkStakeholderEmailFailedInput,
): Promise<void> {
	...
	if (!persisted) {
		logger.info(
			"[publishing-stakeholder-email] failure marker skipped; attempt was already terminal",
