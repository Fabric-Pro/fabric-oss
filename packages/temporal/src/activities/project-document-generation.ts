...
export async function generateDocumentWithAgent(params: {
	projectId: string;
	/**
	 * ProjectDocument id the generated content will be saved to. Optional so
	 * a caller can generate content without persisting it to a
	 * ProjectDocument. When absent, the agent will not receive `documentId`
	 * or an `activeSkill` and the write_document_asset tool won't be bound.
	 */
	documentId?: string;
	documentType: string;
	prompt: string;
...
${formattingRules}`;
		...
		try {
			...
			// Only runs when a concrete documentId is provided (callers that
			// don't persist to a ProjectDocument skip this).
