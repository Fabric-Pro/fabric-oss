...
describe("generateCaseStudyActivity — contributor override", () => {
...
describe("generateCaseStudyActivity — contributor fence scope", () => {
...
describe("generateCaseStudyActivity — the write surface", () => {
	...
	it("imports ONLY the reads and the two writes the guarantee allows", () => {
		expect(
			...
			// The topic's effective Planning & Analysis: a READ, and the only
			// reader that knows about an author's revision. Added deliberately,
			// as this guard requires (Fizzy #1851).
...
describe("generateCaseStudyActivity — the effective analysis reaches the prompt", () => {
...
describe("generateCaseStudyActivity — the settled-decisions block", () => {
	...
	it("admits a settled CODEBASE_DETAIL decision — this type's approval rule names an implementation claim", async () => {
		...
		expect(settledSection()).toContain(
			'- "how much of the resolver to describe" - asked: "What may the piece say about how much of the resolver to describe?" - answered: "Describe the caching layer; do not name internal services."',
		);
	});
	...
	const QUESTION_NOT_SHOWN_WARNING =
		"[publishing-case-study] settled-decisions entries shown cut or without their question";
