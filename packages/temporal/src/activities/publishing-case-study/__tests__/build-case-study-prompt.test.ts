...
describe("the SOURCE DATA fence around interpolated values", () => {
	...
	it("keeps a marker out of a locked-clause bullet as well", async () => {
		...
		// opener there would turn the rules beneath it into quoted source data.
		const prompt = await renderDefault({
			restrictedSubjects: [
...
describe("composeCaseStudyPrompt — refinement (Fizzy #1851, A7)", () => {
	const base = {
		topic: TOPIC,
		context: EMPTY_CONTEXT,
		analysisProse: "",
	...
	it("says NOTHING about revising when there is no draft", async () => {
		const composed = await composeCaseStudyPrompt(base);
	...
	it("leaves a generation prompt byte-for-byte unchanged", async () => {
		...
		const generated = await composeCaseStudyPrompt(base);
		expect(generated.prompt).toBe(
			`Write about Faster incremental builds.\n\n${buildCaseStudyLockedClauses({ restrictedSubjects: [], openQuestionSubjects: [] })}`,
		);
	});
	...
	it("reaches the model even when the bound template ignores every variable", async () => {
		...
		const composed = await composeCaseStudyPrompt({
			...base,
			templateBody: "Write a case study.",
			currentDraft: "Builds used to start cold.",
			guidance: "Make it shorter.",
		});
		expect(composed.prompt).toContain("Builds used to start cold.");
