/**
 * The failure marker for a case study attempt (#1854, Phase 2C).
 *
...
export interface MarkCaseStudyFailedInput {
	draftId: string;
	projectId: string;
	message: string;
}

export async function markCaseStudyFailedActivity(
	input: MarkCaseStudyFailedInput,
): Promise<void> {
	...
	if (!persisted) {
		logger.info(
			"[publishing-case-study] failure marker skipped; attempt was already terminal",
