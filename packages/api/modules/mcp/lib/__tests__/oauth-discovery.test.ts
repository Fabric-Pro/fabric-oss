...
describe("OAuth Discovery", () => {
	...
	describe("Atlassian MCP server (real-world fixture)", () => {
		// Verified 2026-05-19 via curl against mcp.atlassian.com.
		// See docs/superpowers/specs/2026-05-19-atlassian-mcp-oauth-fix-design.md §3.
		const atlassianAuthServerMetadata = {
		...
		it("fetchAuthorizationServerMetadata returns the right Atlassian endpoints from RFC-8414", async () => {
			mockFetch.mockResolvedValueOnce({
				ok: true,
				json: () => Promise.resolve(atlassianAuthServerMetadata),
			});
		...
		it("discoverOAuthEndpoints falls through RFC-9728 (404) to RFC-8414 (200) for Atlassian", async () => {
			// 1st call: RFC 9728 protected-resource metadata — Atlassian returns 404.
			mockFetch.mockResolvedValueOnce({
				ok: false,
				status: 404,
			});
			// 2nd call: RFC 8414 authorization-server metadata — 200 with the fixture.
			mockFetch.mockResolvedValueOnce({
				ok: true,
				json: () => Promise.resolve(atlassianAuthServerMetadata),
			});
			...
			expect(result.hasProtectedResourceMetadata).toBe(false);
			expect(result.hasAuthServerMetadata).toBe(true);
			expect(result.metadata?.registrationEndpoint).toBe(
				"https://cf.mcp.atlassian.com/v1/register",
			);
			expect(result.metadata?.authorizationEndpoint).toBe(
				"https://mcp.atlassian.com/v1/authorize",
			);
			expect(result.metadata?.tokenEndpoint).toBe(
				"https://cf.mcp.atlassian.com/v1/token",
			);
		});
	});
