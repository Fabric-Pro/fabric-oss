// MIRROR of `apps/web/modules/saas/projects/lib/append-attachments-section.ts`.
// Keep both in sync. See spec § 8.2 / Task 1.5 of the chat-thread image-attachments feature.
//
// The server-side copy lives here so `attach-pending-media-to-story.ts` (in
// `@repo/api`) can import the helper at apply time without dragging in
// `apps/web` imports (the workspace DAG forbids that edge). This mirrors the
// precedent set by `extract-story-media-keys.ts` in this same directory.
 ...
 * prevent.
 *
 * Shared with `attach-pending-media-to-story.ts`, which fills the warning-line
 * slot inside the same block and must agree on what "already has a section"
 * means.
 */
export function hasAttachmentsHeading(target: string): boolean {
	return target
		.split("\n")
		.some((line) =>
 ...
 *
 * Exported because the "Update using context" re-injection path needs the same
 * decision and had grown its own copy of it. One implementation means the two
 * paths cannot drift on whether a second heading gets opened, or on the spacing
 * that separates the block from the body.
 */
export function mergeEntriesInto(target: string, entries: string): string {
