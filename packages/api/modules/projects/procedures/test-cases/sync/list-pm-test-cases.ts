...
} from "../../stories/sync/gitlab-pm-adapter";
import type {
	PMTicketListError,
	PMTicketListNote,
} from "../../stories/sync/list-pm-tickets-filters.types";

/**
 * AUTHORIZATION: requireProjectPermission(TEST_CASE_READ) — any member who can
 * read the project (viewers included) may list importable PM items.
 *
 * Lists unsynced tickets from the connected PM tool for the given project.
 * Returns only items whose external ID does not already exist as a Fabric test
 * case in this project, so the user sees only new importable items.
 *
...
const inputSchema = z.object({
	 ...
	 * Ids capped at 200 as defense-in-depth (client already expands and caps
	 * before the request).
	 */
	filters: z
		.object({
...
async function handleGitLabList(args: {
	...
	const existingRows = await db.testCase.findMany({
		// `deletedAt: null`: a soft-deleted case must not keep its PM ticket
		// flagged "already imported" — the ticket should be re-importable.
	...
	return {
		tickets: pageItems.map((item) => ({
			...
			state: item.state,
			// See the MCP path: match id OR displayId so an imported case (whose
			// stored `externalId` is the display id) still shows as synced.
			alreadySynced:
				existingExternalIds.has(item.id) ||
				existingExternalIds.has(item.displayId),
		})),
		total,
		totalOnBoard,
		alreadySynced,
...
export const listPmTestCasesProcedure = tenantProtectedProcedure
	.use(requireProjectPermission(Permissions.TEST_CASE_READ))
	.route({
		method: "GET",
		path: "/projects/{projectId}/pm-test-cases",
		tags: ["Projects", "Test Cases", "Sync"],
		summary: "List importable PM test cases",
		description:
	...
	.handler(async ({ input, context }) => {
		...
		// Collect external IDs already imported into this project (used to filter
		// below). `deletedAt: null`: a soft-deleted case must not keep its PM
		// ticket flagged "already imported" — the ticket should be re-importable.
		const existingRows = await db.testCase.findMany({
			where: {
				projectId: input.projectId,
				externalId: { not: null },
		...
		if (isADO && hasRequestedIds) {
			...
			// first ≤5 requested ID values (PII guardrail).
				...
				try {
					...
					if (capabilities.detectedType === "fizzy") {
						const fizzyResult = await listAllFizzyCards({
							...
							organizationId: project.organizationId ?? undefined,
							capabilities,
							// Same as the story picker: the returned rows carry no
							// description, and selected cases are imported through
							// `fetchPMItemsByIds`, which re-fetches each card in
							// full. Nothing downstream of this listing reads the
							// dropped fields.
							fields: "summary",
						});
						if (fizzyResult) {
		...
		return {
			tickets: pageItems.map((item) => ({
				...
				workItemType: item.workItemType,
				state: item.state,
				// Match on BOTH the internal id and the displayId: some PM tools
				// (e.g. Fizzy) key list rows by an internal id but are fetched /
				// updated / imported by their display number, so an imported case's
				// stored `externalId` is the displayId. Checking only `item.id`
				// would hide the "Imported" badge for those.
				alreadySynced:
					existingExternalIds.has(item.id) ||
					existingExternalIds.has(item.displayId ?? item.id),
			})),
			total,
			totalOnBoard,
			alreadySynced,
