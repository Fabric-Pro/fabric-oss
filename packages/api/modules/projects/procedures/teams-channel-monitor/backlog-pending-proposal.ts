 ...
 * or rejected. The associated seen-message markers remain, so the source thread
 * will not be re-analyzed (parity with reject).
 */
export const backlogPendingProposalProcedure = tenantProtectedProcedure
	.use(requireProjectPermission(Permissions.PROJECT_UPDATE))
	.route({
		method: "POST",
		path: "/projects/{projectId}/teams-channel-monitor/pending-proposals/{proposalId}/backlog",
