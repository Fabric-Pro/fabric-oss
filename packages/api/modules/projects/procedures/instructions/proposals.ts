...
export const MAX_PROPOSAL_DIFF_INLINE_BYTES =
...
export const getInstructionProposalFileProcedure = tenantProtectedProcedure
	...
	.handler(async ({ input, context }) => {
		...
		if (
			...
			throw new ORPCError("NOT_FOUND", { message: "Proposal file not found" });
		}
		const base = await getInstructionSnapshot(
		...
		const { data } = await getStorageProvider().downloadFile(file.storageKey, {
			bucket: BUCKET,
		});
