...
// Extensions inline-classified here (before a file has ever touched the
// database) purely to decide MIME type and the `isText` RENDERING hint —
// NOT the `@repo/instructions` `classifyPath` taxonomy, which drives `kind`.
//
...
// and the secret scan reading it that way is precisely how those files
