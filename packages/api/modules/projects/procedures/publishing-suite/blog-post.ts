 ...
 * that function lives in `@repo/temporal`, which the API package does not and
 * should not depend on for a string join. The pairing is pinned by a test
 * asserting both produce the same text for the same document — a duplicated
 * rule with nothing checking the copies is how the two silently diverge and an
 * adopted version stops matching the one that was seeded.
 */
