...
vi.mock("@repo/database", () => ({
	...
	// Exported ONLY so the assertion below can name it. Read its caveat there
	// before trusting it.
...
describe("setQuestionAssignees", () => {
	...
	/**
	 * A WEAK guard, deliberately kept and deliberately labelled.
	 *
	 * `set-question-assignees.ts` does not import `answerTopicQuestion`, so this
	 * spy cannot fire whatever the handler does — it would stay green if a
	 * future change settled the root through some other path. It is here to
	 * document the invariant at the surface a reader looks at, not to enforce
	 * it.
	 *
