 ...
 */
describe("generateCaseStudy — refine", () => {
	const SAVED_BODY = "# Cutting release lead time";
	...
	it("takes the body from the SERVER's read, not from the caller", async () => {
		...
		expect(workflow).toBe("generatePublishingCaseStudyWorkflow");
		expect(options.args[0]).toEqual(
			expect.objectContaining({
	...
	it("reads the draft BEFORE opening the attempt row", async () => {
		...
		).rejects.toMatchObject({ message: "No saved case study to refine." });
