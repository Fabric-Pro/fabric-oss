 ...
 */
describe("generateStakeholderEmail — refine", () => {
	const SAVED_BODY = "Subject: Release update";
	...
	it("takes the body from the SERVER's read, not from the caller", async () => {
		...
		expect(workflow).toBe("generatePublishingStakeholderEmailWorkflow");
		expect(options.args[0]).toEqual(
			expect.objectContaining({
	...
	it("reads the draft BEFORE opening the attempt row", async () => {
		...
		await expect(
			...
			message: "No saved stakeholder email to refine.",
		});
