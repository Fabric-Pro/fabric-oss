/**
 * LinkedIn Post — start one generation run, and save the chosen option
 * (Fizzy #1851).
 *
 * Both writes; the read side is `listTopicDrafts`, polled while a run is in
 * flight. Scoped exactly like `generateShortPost`: neither procedure trusts a
 ...
 *
 * Deliberately a sibling of `short-post.ts` rather than a shared implementation
 * parameterised by post type. The two are the same shape today, but they are the
 * same shape by coincidence of both being candidate-based, not by contract: the
 * option-text bound already differs (LinkedIn's ceiling is around 3,000
 * characters, X's is 280), and the family has form for the two halves of a
 * "shared" generator diverging — blog post seeds a working draft on its first
 * run where this one deliberately does not. A parameterised pair would make the
 * next such difference a flag deciding which product it is.
 */
...
export const generateLinkedInPostProcedure = tenantProtectedProcedure
	.use(requireProjectPermission(Permissions.PUBLISHING_TOPIC_UPDATE))
	.route({
		method: "POST",
		path: "/projects/{projectId}/publishing-topics/{topicId}/linkedin-post",
		tags: ["Projects", "Publishing Suite"],
		summary: "Generate LinkedIn post options for a topic",
	})
	.input(
		z.object({
			projectId: z.string(),
			...
			/**
			 * Revise the saved working LinkedIn post rather than draft a new one
	...
	.handler(async ({ input, context }) => {
		...
		// is derived from the loaded Project row, and `input.organizationId` is a
		// guard only — never a scoping key.
		...
		try {
			await client.workflow.start(
				"generatePublishingLinkedInPostWorkflow",
				{
					...
					workflowId: `publishing-topic-li:${attempt.draftId}`,
			...
			if (!rollback.persisted) {
				logDraftRefusal(
					"[publishing-linkedin-post] start rollback skipped",
					rollback.reason,
					{ draftId: attempt.draftId, projectId: project.id },
				);
			}
			throw new ORPCError("INTERNAL_SERVER_ERROR", {
				message: "Could not start the LinkedIn post",
			});
		}
...
/**
 * Adopt one generated option as the topic's working LinkedIn post.
 *
 ...
 * actually produced.
 */
export const selectLinkedInPostOptionProcedure = tenantProtectedProcedure
	.use(requireProjectPermission(Permissions.PUBLISHING_TOPIC_UPDATE))
	.route({
		method: "POST",
		path: "/projects/{projectId}/publishing-topics/{topicId}/linkedin-post/select",
		tags: ["Projects", "Publishing Suite"],
		summary: "Save a generated LinkedIn post option as the working draft",
	})
	.input(
		z.object({
			projectId: z.string(),
			 ...
			 * for "nothing is saved". Optimistic concurrency: without it, two
			 * people choosing different options seconds apart both succeed, with
			 * the second silently erasing the first.
			 *
			 ...
			 * while the row had changed.
			 *
			 * Optional on the wire so an older client still works; absent means
			 * "do not check". The clients in this repo always send it.
			 */
	...
	.handler(async ({ input, context }) => {
		...
		if (lookup.status === "ambiguous") {
			...
			throw new ORPCError("INTERNAL_SERVER_ERROR", {
				message:
					"This draft has more than one option under that label, so the selection is ambiguous. Regenerate the LinkedIn post.",
			});
		}
		...
		const saved = await saveWorkingDraft({
			...
			expectedUpdatedAt:
				input.expectedUpdatedAt === undefined
					? (workingDrafts.find((w) => w.postType === "LINKEDIN_POST")
							?.updatedAt ?? null)
		...
		if (saved.status === "stale") {
			...
			throw new ORPCError("CONFLICT", {
				message:
					"The saved LinkedIn post changed while you were choosing. Refresh and try again.",
			});
		}
		...
		// Measurement only (Fizzy #1851 A9). Adopting an option verbatim is the
		// cleanest acceptance signal the Suite produces — the text saved IS the
		// text the model wrote, since this endpoint refuses to take a body.
		await recordPublishingOutcome({
			outcome: "ACCEPTED_AS_IS",
			subjectType: "publishing-linkedin-post",
...
/**
 * The result of resolving a label against a stored LinkedIn post document.
 *
...
/**
 * Find one option inside a stored LinkedIn post document.
 *
 ...
 * `PublishingLinkedInPostSchema` refuses to persist a run whose labels collide,
 * so nothing this code writes can reach it. It is here because `content` is a
 * JSON column and this function already declines to trust any other part of its
