...
const BODY_MAX = WORKING_DRAFT_BODY_MAX.CASE_STUDY;
...
export const generateCaseStudyProcedure = tenantProtectedProcedure
	...
	.input(
		z.object({
			...
			/**
			 * Revise the saved working case study rather than draft a new one
	...
	.handler(async ({ input, context }) => {
		...
		try {
			...
			if (!rollback.persisted) {
				logDraftRefusal(
					"[publishing-case-study] start rollback skipped",
					rollback.reason,
...
export const adoptCaseStudyDraftProcedure = tenantProtectedProcedure
	...
	.handler(async ({ input, context }) => {
		...
		const candidate =
			caseStudy?.versions.find((v) => v.id === input.draftId) ?? null;
		if (!candidate) {
