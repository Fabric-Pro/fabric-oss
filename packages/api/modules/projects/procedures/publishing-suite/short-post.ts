...
export const generateShortPostProcedure = tenantProtectedProcedure
	...
	.input(
		z.object({
			...
			/**
			 * Revise the saved working short post rather than draft a new one
...
export const selectShortPostOptionProcedure = tenantProtectedProcedure
	...
	.handler(async ({ input, context }) => {
		...
		const candidate =
			tweet?.versions.find((v) => v.id === input.draftId) ?? null;
		if (!candidate) {
...
/**
 * How long a saved short post may be.
 *
...
const SHORT_POST_BODY_MAX = WORKING_DRAFT_BODY_MAX.TWEET;

/**
 * `publishingSuite.saveTweetBody` — save a hand edit to the adopted
 * short post.
 *
 ...
 * overwritten.
 */
export const saveTweetBodyProcedure = tenantProtectedProcedure
	.use(requireProjectPermission(Permissions.PUBLISHING_TOPIC_UPDATE))
	.route({
		method: "POST",
		path: "/projects/{projectId}/publishing-topics/{topicId}/short-post/body",
		tags: ["Projects", "Publishing Suite"],
		summary: "Save an edit to the working short post draft",
	})
	.input(
		z.object({
			projectId: z.string(),
			topicId: z.string(),
			organizationId: z.string().nullable().optional(),
			body: z.string().min(1).max(SHORT_POST_BODY_MAX),
	...
	.handler(async ({ input, context }) => {
		...
		if (saved.status === "not_found") {
			throw new ORPCError("NOT_FOUND", {
				message: "No saved short post to edit",
			});
		}
		if (saved.status === "stale") {
			throw new ORPCError("CONFLICT", {
				message:
					"The saved short post changed while you were editing. Refresh and try again.",
			});
		}
