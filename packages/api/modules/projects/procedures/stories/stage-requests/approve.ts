/**
 * Approve Stage Transition Request
 *
 ...
 * `ProjectStageApprover` and forbids self-approval — both surface as
 * FORBIDDEN via `mapStageTransitionError`. Readiness is re-evaluated at
 * approval time inside the same transaction that flips the request.
 */
...
export const approveStageRequestProcedure = tenantProtectedProcedure
	.use(requireProjectPermission(Permissions.STORY_STAGE_APPROVE))
	.route({
		method: "POST",
		path: "/projects/{projectId}/stories/stage-requests/{requestId}/approve",
		tags: ["Projects", "Features", "Governance"],
		summary: "Approve a drafting-stage transition request",
	})
	.input(
		z.object({
			projectId: z.string(),
