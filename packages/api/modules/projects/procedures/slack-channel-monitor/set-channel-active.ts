...
import {
	db,
	deactivateLinkedSlackChannel,
	reactivateLinkedSlackChannel,
} from "@repo/database";
import { z } from "zod";
...
export const setChannelActiveProcedure = tenantProtectedProcedure
	...
	.route({
		method: "POST",
		path: "/projects/{projectId}/slack-channel-monitor/set-active",
		tags: ["Projects", "Slack Channel Monitor"],
		summary: "Pause or resume scanning a linked Slack channel",
		description:
	...
	.handler(async ({ input, context }) => {
		...
		const channel = await db.projectLinkedSlackChannel.findFirst({
		...
		const updated = input.active
			? await reactivateLinkedSlackChannel({
					projectId: input.projectId,
					linkedChannelId: input.linkedChannelId,
				})
			: await deactivateLinkedSlackChannel({
					projectId: input.projectId,
					linkedChannelId: input.linkedChannelId,
					userId: user.id,
				});
		...
		recordAuditFromRequest(context, {
			...
			projectId: input.projectId,
			resource: {
				type: "linked_slack_channel",
				id: input.linkedChannelId,
			},
			metadata: { provider: "SLACK_CHANNEL", active: input.active },
		});
