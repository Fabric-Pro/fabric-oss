/**
 * v1 Synced context files (Fizzy #2618)
 *
 ...
 * validation, the write, the embed and the audit row; this file is
 * authorization and the wire contract only.
 *
 ...
 *     projects:write" }`.
 *  2. The key creator's LIVE `CONTEXT_CREATE` on the project, re-checked on
 *     every call — a wildcard `*` key included, since `hasScope` waves `*`
 *     through the first gate — through the same resolver the oRPC twin's
 *     `requireProjectPermission(Permissions.CONTEXT_CREATE)` uses. A refusal
 *     is the nested `{ error: { message } }`, so a scope problem and a
 *     permission problem never read as each other.
 *
...
import { syncedContextConflictMessage } from "../projects/lib/synced-context-conflict";
 ...
 *  6. `CONTEXT_CREATE`. A personal-project owner would pass unconditionally,
 *     but step 3 has already refused every personal project.
...
async function resolveContextProject(
	...
	if (!hasPermission(access.permissions, Permissions.CONTEXT_CREATE)) {
		return {
			error: forbidden(
...
/**
 * The status for one of `upsertSyncedContext`'s own refusals, or `null` for
 * anything else. It signals with `ORPCError`; two codes are reachable from
 * here — BAD_REQUEST for the caller's input, FORBIDDEN for a project with no
 * organization — and anything else (a Prisma or storage error) propagates to
 * the app's error handler rather than being given a status it did not earn.
 */
function upsertFailureStatus(error: unknown): 400 | 403 | null {
...
export function registerContextRoutes(
	...
	app.put(
		...
		async (c) => {
			...
			const resolved = await resolveContextProject(projectId, apiCtx, {
				org: c.req.query("org"),
			...
			try {
				...
				const status = upsertFailureStatus(error);
				if (status === null) {
					throw error;
				}
				const message =
