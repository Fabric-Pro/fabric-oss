...
import { db, listNewsletterSubscribers } from "@repo/database";
import { z } from "zod";
...
export const listSubscribersProcedure = tenantProtectedProcedure
	...
	.handler(async ({ input }) => {
		...
		const subscribers = await listNewsletterSubscribers(input.projectId);
		return { subscribers };
	});
