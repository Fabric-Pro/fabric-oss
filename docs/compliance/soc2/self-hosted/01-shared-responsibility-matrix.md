...
| CC6.6 Encryption in transit | **F+C** | Prod Helm values enforce HTTPS (ALB `ssl-redirect: 443`, ACM cert). The Postgres client and both Temporal connections (API client and worker) refuse plaintext in production but retain named emergency overrides. Service-to-service traffic inside the cluster is encrypted by the node hardware: Terraform refuses node types that do not encrypt traffic between instances. The ALB-to-pod hop is not covered by that, and Postgres TLS does not yet verify the server certificate (see the reference architecture, *Internal traffic*). The customer provides the certificate/domain, restricts override variables, and chooses how to encrypt the ALB-to-pod hop (VPC Encryption Controls or TLS in the pod). |
