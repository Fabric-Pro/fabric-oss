...
| CC6.6 Encryption in transit | **F+C** | Prod Helm values enforce HTTPS (ALB `ssl-redirect: 443`, ACM cert). The Postgres client and both Temporal connections (API client and worker) refuse plaintext in production but retain named emergency overrides. Service-to-service traffic inside the cluster is encrypted by the node hardware: Terraform refuses node types that do not encrypt traffic between instances. Postgres TLS verifies the RDS certificate against the Amazon RDS CA bundle the chart mounts. The ALB-to-pod hop is not covered by the node encryption (see the reference architecture, *Internal traffic*). The customer provides the certificate/domain, restricts override variables, and chooses how to encrypt the ALB-to-pod hop (VPC Encryption Controls or TLS in the pod). |
