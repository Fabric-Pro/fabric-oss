...
| `no pg_hba.conf entry for host ..., no encryption` (SQLSTATE 28000) | The connection went out without TLS; RDS rejects that | `DATABASE_URL` should carry `sslmode=verify-full&sslrootcert=/etc/fabric/rds/global-bundle.pem` (see `ENVIRONMENT-VARIABLES.md` §3.1). The chart's `PGSSLMODE=no-verify` only covers a URL with no `sslmode`. |
| `[security] Refusing to start: production DATABASE_URL must require TLS` | The database secret predates the verify-full URLs (`?schema=public` only) | Update `DATABASE_URL` / `DIRECT_URL` / `WORKER_DATABASE_URL` to the §3.1 forms, then `helm upgrade`. |
| `unable to verify the first certificate` / `self-signed certificate in certificate chain` / `P1011: Error opening a TLS connection` | The RDS CA bundle is not mounted, or the URL points at a different path | Check the pod mounts `fabric-rds-ca` at `/etc/fabric/rds` (`kubectl get configmap fabric-rds-ca -n fabric`) and that the URL's `sslrootcert` / `sslcert` path is `/etc/fabric/rds/global-bundle.pem`. |
