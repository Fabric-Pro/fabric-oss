...
| `APPLICATIONINSIGHTS_CONNECTION_STRING` | Vercel env (staging + prod) | no | Enables `web`'s App Insights log forwarding — every `logger.warn`/`error`/`fatal` line, plus browser-reported oRPC transport/error-page failures (`/api/client-errors`). **Independent of `FABRIC_FEATURE_BURN_RATE_ALERTS`**, unlike `trackEvent`/`trackMetric`: log forwarding has no kill switch of its own and no-ops cleanly (a warning, not an error) when unset or malformed. This is what lets `web`'s backend logs reach the same Application Insights resource the other Azure-hosted services already report to — `web` runs on Vercel rather than in the Azure Container Apps estate the other services share, so without a connection string here its logs are reachable only through Vercel's own runtime log viewer. |
