...
### Reach record
The runtime's own note that one credential carried a request into an organization over MCP. It says a request arrived and which credential proved it — nothing about what sent it. The protocol offers a client to name itself at handshake, and the runtime neither reads nor stores that, so a coding tool, a desktop assistant and a script are one and the same fact here. Written by the server for itself at the point it has already decided the request is legitimate — the secret matched, the owner was loaded, membership was re-read — and never for a browser session or an anonymous caller.
...
### First reach
The permanent mark that an organization was reached over MCP for the first time, kept apart from the Reach record on purpose and never invalidated afterwards.
...
An organization that can currently be reached over MCP: at least one of its Reach records still names a credential that is active, unexpired, and held by someone who is still a member. Says nothing about what does the reaching — see Reach record.
...
return unauthorized. Only a Reach record says something arrived; everything else says
