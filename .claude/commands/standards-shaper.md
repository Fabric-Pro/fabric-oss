...
- `fabric/standards/` (primary location)
- `.claude/` or `.factory/` configuration
- Project root for framework indicators

```bash
# Check what exists
ls fabric/standards/ 2>/dev/null || echo "No standards directory yet"
```
...
- `Cargo.toml` → Rust
- `composer.json` → PHP
- Database config files → PostgreSQL, MySQL, MongoDB, Cosmos DB, etc.
...
Create `fabric/standards/README.md`:
...
- **[coding-style.md](global/coding-style.md)** - Core coding style and principles
- **[conventions.md](global/conventions.md)** - Naming and structural conventions
- **[error-handling.md](global/error-handling.md)** - Error handling patterns
- **[validation.md](global/validation.md)** - Input validation standards
...
📁 Standards Location: fabric/standards/
...
Frontend:
  ...
  ✅ css.md - Updated with Tailwind CSS v4 examples
...
1. Review updated standards in fabric/standards/
...
🔗 Workflow Guide: fabric/standards/RECOMMENDED_WORKFLOW.md
```
...
(List other categories as created)

📊 Research Used:
  Exa Code Context, Ref Documentation, Codebase Analysis, Framework Knowledge

💡 Next Steps:

1. Review the standards in fabric/standards/
...
3. Database (PostgreSQL, MongoDB, Cosmos DB, etc.)
```
