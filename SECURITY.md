...
- **Sanitization rules** (`js/incomplete-*-sanitization`, `js/bad-tag-filter`, `js/double-escaping`, `js/xss-through-dom`) — real only when the result reaches an HTML or DOM sink, a host-trust decision, or a shell. A `.replace()` that formats text for a prompt, a markdown document, a log line, a slug, a hash input or a JSON field is a false positive, and the dismissal names that sink. A substring host check that gates trust is fixed by parsing the URL and comparing the hostname exactly. When a `js/double-escaping` fix is only reordering the replacement chain (decode `&amp;` last), it is fixed regardless of sink: the change is behaviour-identical for every singly-escaped input. At an HTML sink, a removal whose deletion can join the text on either side into a new match (`js/incomplete-multi-character-sanitization`) is repeated until the output stops changing, under a pass cap past which the input is dropped rather than rendered half-sanitised. That is necessary for a deletion-based filter but not sufficient: the filter runs on the final string that reaches the sink, not on pieces later joined into it, and URL checks read the final attribute values. A filter that does not parse CSS does not make CSS safe; what a stylesheet can fetch is contained by the renderer (no script, no network), and the fix says so rather than claiming the filter covers it.
- **`js/incomplete-url-scheme-check`** — real when the checked URL is written into rendered HTML: the check then treats `javascript:`, `vbscript:` and `data:` alike, reads the scheme the way a browser does (character references decoded, whitespace and control characters dropped, lower-cased), and keeps a `data:` URL only for an allow-listed raster image type. A false positive when the scheme test only chooses between outputs that are never rendered or followed (a log line, a label); the dismissal names that sink.
- **`js/bad-code-sanitization`** — real when a caller-, tenant- or model-influenced value is `JSON.stringify`-ed into script text: the fix escapes `<`, `>`, `/`, U+2028 and U+2029 after stringifying, or passes the value as data (an attribute, `postMessage`) instead of code. A false positive when every stringified value is a module constant; the dismissal names the constant.
...
- **`js/identity-replacement`** — fixed outside tests: a replacement that maps its match to itself does nothing, so the intended replacement is restored or the call removed.
...
| `GHSA-7pqw-9j4j-h8q3` | `extract-zip@2.0.1` via `@langchain/langgraph-cli` → (`create-langgraph`) | Arbitrary file write through symlink archive entries (CWE-59), published 2026-09 — the **same defect class on the same package** as `GHSA-jmr9-qjv8-65gv` above. Chain, unfixability and reachability are identical and are deliberately not restated: `2.0.1` is still both the last affected and the newest published version, `@langchain/langgraph-cli` still declares `extract-zip: ^2.0.1` (re-confirmed in the lockfile 2026-09-09, resolving through `create-langgraph@1.1.5`), and the sole import remains the CLI's Windows-only Python `langgraph dev` path, which no deployed agent image builds or runs. Given the same expiry as its sibling on purpose: one upstream fix retires both, and staggered dates would mean repeating a single investigation twice. | 2026-11-12 |
