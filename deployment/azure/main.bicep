...
@description('Living Documents auto-refresh (FABRIC_FEATURE_LIVING_DOCS_REFRESH) on the temporal worker. TRUE in every environment, prod included — this is deliberately NOT the rollout switch. Rollout is owned solely by the web app\'s own copy of the flag (Vercel): with it off nobody can enroll a document, and an empty due-list makes the hourly sweep a no-op. What this param buys is the kill switch — the worker re-reads the flag immediately before it writes, so setting it false stops an AI mid-rollout. Set false only to hit the brakes.')
