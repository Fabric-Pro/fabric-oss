...
@description('Encryption key version that NEW data is encrypted with (SOC 2 CC6.1 key rotation). Empty leaves rotation inactive, which is a fully supported setup: encryption falls back to BETTER_AUTH_SECRET and every existing ciphertext still decrypts. Nothing in this repository sets it; the deployment pipeline sets the version each of its environments is on. Set this only once the environment\'s Key Vault actually holds that version\'s material, because activating a version whose material is absent fails EVERY stored-credential read (the lookup is lazy, so the process boots clean and then poisons the work it accepts). Defaults empty so a freshly-provisioned environment is never born broken. See your operator\'s key-rotation runbook.')
...
var temporalWorkerBaseEnv = [
  ...
  // The deployment pipeline's copy takes it from a parameter too, but its
  // environment-bootstrap path still passes the non-prod version, so a new
  // environment it creates needs that key material seeded before first deploy.
